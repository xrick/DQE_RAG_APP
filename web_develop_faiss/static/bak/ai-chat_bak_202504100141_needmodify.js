...
(function () {
    ...
    // 新增動畫控制函數
    function startCustomAnimation(baseText, type = 'dot_pulse') {
        const element = document.getElementById('custom-animation');
        if (!element) return;
        ...
        // 旋轉圖標動畫
        else if (type === 'rotating_icon') {
            const icons = ['◜', '◝', '◞', '◟'];
            let iconIndex = 0;
            currentAnimationInterval = setInterval(() => {
                iconIndex = (iconIndex + 1) % 4;
                element.textContent = `${baseText} ${icons[iconIndex]}`;
            }, 200);
        }
    }
            ...
            while (true) {
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (jsonData.type === "animation") {
                            // 停止舊動畫
                            stopCustomAnimation();
                            // 啟動新動畫
                            startCustomAnimation(
                                jsonData.base_text || "合併資料產生中",
                                jsonData.animation_type || "dot_pulse"
                            );
                        }
